...
class ParserTestCase(unittest.TestCase):
    ...
    def test_callable_ambiguity(self):
        scope = {'add':lambda x, y:x+y}
        self.assertEquals(2, dojo_compile('add\n(5, 2)')(scope))

    def test_callable_ambiguity_not(self):
        scope = {'add':lambda x, y:x+y}
        self.assertEquals(2, dojo_compile('add,(5, 2)')(scope))
    ...
    def test_import_module(self):
        scope = Scope()
        self.assertEquals(__import__('math'), dojo_compile('import math')(scope))
        self.assertEquals(__import__('math'), scope.get('math'))
        
    def test_import_module_items(self):
        scope = Scope()
        math = __import__('math')
        self.assertEquals(math, dojo_compile('import math(sqrt, cos, log)')(scope))
